...
import logo from "../../assets/images/logo-special.png";
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className= 'bg-[#F0F9FF] px-4 sm:px-16 w-full items-center fixed top-0 z-10'
    >
      <div className="sm:container mx-auto sm:px-4 py-4">
        <div className="flex justify-between sm:items-center">
          ...
          <ul className="sm:flex hidden sm:space-x-6 text-sm text-gray-700">
            {menuKeys.map((key) => (
              <li
                key={key}
          ...
          <div className="flex space-x-4">
            <button
              onClick={openModal}
              className="px-3 md:px-5 py-3 text-sm md:text-sm cursor-pointer text-[#0129A4] border border-[#0129A4] bg-opacity-30 p-2 rounded hidden sm:block"
            >
              {t("cta")}
            </button>
            <button
              onClick={() => scrollToSection("request-quote")}
              className="text-white hidden sm:block px-2 md:px-5 py-3 text-xs md:text-sm font-bold bg-[#0129A4] rounded"
            >
              {t("request")}
            </button>
            <button
              onClick={toggleSidebar}
              className="sm:hidden text-grey-800 mt-2 text-2xl"
            >
